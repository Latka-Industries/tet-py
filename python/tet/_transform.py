...
path : str or path-like, optional
    Spill file path; omit for an engine temp file under the allowlist.

Returns :class:`~tet.SpillTransformResult` with ``.path`` to dense row-major LE bytes.

"""

_SIDECAR_EXTRA = """\
path : str or path-like, optional
    Sidecar ``.tet`` path; default beside the source file when implemented.

Returns :class:`~tet.SidecarTransformResult` (``.open()`` → :class:`~tet.TetFile`).

Note: tetration ``write: sidecar`` is not implemented yet.
...
class TransformOps:
    ...
    def _run(
        ...
        *,
        ...
        doc["write"] = write_to_wire(write, path)
...
class _NumpySink(_TransformSink):
    """``f.transform.to_numpy`` — in-process / preview (wire ``ram``)."""


class _SpillSink(_TransformSink):
    """``f.transform.to_spill`` — dense spill file (wire ``spill``)."""


class _SidecarSink(_TransformSink):
    """``f.transform.to_sidecar`` — ``.tet`` sidecar (wire ``sidecar``; engine TBD)."""
